...
class BaseStage(BaseInstrument):
    ...
    def _validate_position(self, attribute, value):
        ...
        if not min_val <= value <= max_val:
            raise ValueError(f"{self.name} position must be between {min_val} and {max_val}")
...
class BaseValve(BaseInstrument):
    ...
    def __call__(self, port) -> bool:
        ...
        try:
            [_port, self._port] = [self._port, port]    # Save current port and validate new port
            return True
        except ValueError as e:
            warn(f'{e}', UserWarning)
            return False
...
class BaseLaser(BaseInstrument):
    ...
    def _validate_power(self, attribute, value):
        ...
        if not min_val <= value <= max_val:
            raise ValueError(f"{self.name} power must be between {min_val} and {max_val}")
...
class BaseCamera(BaseInstrument):
    ...
    def validate_exposure(self, attribute, value):
        ...
        if not min_val <= value <= max_val:
            raise ValueError(f"{self.name} exposure must be between {min_val} and {max_val}")
...
class BaseTemperatureController(BaseInstrument):
    ...
    def validate_temperaute(self, attribute, value):
        ...
        if not min_val <= value <= max_val:
            raise ValueError(f"{self.name} temperature must be between {min_val} and {max_val}")
