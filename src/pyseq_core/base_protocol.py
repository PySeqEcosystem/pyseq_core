...
from pydantic import BaseModel, create_model, computed_field, PositiveInt, PositiveFloat, field_validator, model_validator
...
def validate_min_max(instrument:str, value: Union[int, float], valid_dict = HW_CONFIG) -> Union[int, float]:
...
class BaseStagePosition(BaseModel):
    ...
    z_step: PositiveInt = HW_CONFIG['ZStage']['step']                           # Not a cached_property because want to change easily
...
def check_pump(flowcell:str, 
               params: Union[dict, int, float],
               PumpCommand: BasePumpCommand) -> dict:
...
def check_image(params: Union[dict, int], 
                ImageParams: BaseModel,
                FocusParams: BaseModel,
                StagePosition: BaseModel) -> dict:
...
def check_expose(params: Union[dict, int], 
                ExposeParams: BaseModel,
                StagePosition: BaseModel) -> dict:
...
def format_protocol(flowcell: str, 
                    protocols: dict, 
                    exp_config:dict
                    ) -> dict:
