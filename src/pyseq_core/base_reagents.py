...
from pyseq_core.base_protocol import validate_in, validate_min_max, custom_params, recursive_validate
import logging
...
class ReagentsManager():
    ...
    def get_reagent_key(self, flowcell: Union[str,int], port: int) -> str:
        ...
        if len(key) > 1:
            raise ValueError(f'More than 1 reagent mapped to port {port} on flowcell {flowcell}')
        return "".join(key)
    ...
    def check_port(self, flowcell: Union[str,int], reagent: BaseReagent) -> bool:
        ...
        if reagent.name in reagents:
            ...
            warn(f'Duplicate Port: {reagent.name} already mapped to port {port} on flowcell {flowcell}', UserWarning)
            return False
        ...
        if len(existing_reagent) > 0: 
            warn(f'Duplicate Port: Port {reagent.port} is already used by {existing_reagent} on flowcell {flowcell}', UserWarning)
            return False
        
        return True
        
    
    def check_flow_rate(self, flowcell: Union[str,int], flow_rate: Union[int, float]) -> bool:
    ...
    def add_from_config(self, flowcell: Union[str, int], config: dict):
        ...
        ExtraPumpParams = create_model('ExtraPumpParams', **custom_params(config['pump']))
