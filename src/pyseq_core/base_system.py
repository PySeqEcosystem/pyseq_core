...
from pyseq_core.base_instruments import (
    BaseYStage, BaseXStage, BaseZStage, BaseObjectiveStage,
    BaseShutter, BaseFilterWheel, BaseLaser, BaseCamera,
    BasePump, BaseValve, BaseTemperatureController
)
from pyseq_core.base_reagents import ReagentsManager
from pyseq_core.base_protocol import Optics, HoldCommand, WaitCommand, TemperatureCommand
from pyseq_core.base_protocol import BaseROIFactory, SimpleStageFactory, PumpCommandFactory
from pyseq_core.base_protocol import read_protocol, format_protocol, need_reagents, check_for_rois, read_user_config
...
class BaseSystem(ABC):
    ...
    async def _worker(self):
        ...
        while not self._loop_stop:
            ...
            if not self._queue_dict[id][1]:
                ...
                self._current_task = asyncio.create_task(func(*args, **kwargs), name=task_name)
                ...
                try: 
                    ...
                    LOGGER.warning(f"{self.name} :: Task {id} :: {description} cancelled")
                except Exception:
                    # Check the task status
                    task_exception = self._current_task.exception()
                    LOGGER.error(f"{self.name} :: Task {id} :: {description}: {task_exception}")        
    ...
    async def _check_pause_and_cancel(self, await_task: asyncio.Task, check_cancel: bool = True):
...
class BaseMicroscope(BaseSystem):
    ...
    @reserve_microscope
    async def _from_flowcell(self, routine: Literal['image', 'focus', 'expose'], 
                             roi: List[BaseModel]):
        for r in roi:
...
class BaseFlowCell(BaseSystem):
    ...
    def pump(self, volume: Union[int, float], 
        ...
        if self.Pump(volume, flow_rate):
            if not reverse:
                ...
                return self.add_task(description, self._pump, volume, flow_rate, **kwargs)
            else:
                return self.reverse_pump(volume, flow_rate, **kwargs)

    def reverse_pump(self, volume: Union[int, float], 
                           flow_rate:Union[int, float],
        ...
        return self.add_task(description, self._reverse_pump, volume, flow_rate, **kwargs)
    ...
    async def _to_microscope(self, routine: Literal['image', 'focus', 'expose'], 
    ...
    def update_protocol_cycle_task(self, cycle: int, total_cycles: int):
        ...
        if total_cycles > 1:
            LOGGER.info(f'Start {cycle}/{total_cycles} of protocol {self._protocol_name}')
...
class BaseSequencer(BaseSystem):
    ...
    def pump(self, flowcells: Union[str,int] = None, 
    ...
    def hold(self, flowcells: Union[str,int] = None, 
                   hold_command: HoldCommand = None, **kwargs) -> Union[int, List[int]]:
    ...
    def wait(self, flowcells: Union[str,int] = None,
                   wait_command: WaitCommand = None, **kwargs) -> Union[int, List[int]]:
    ...
    def _roi_to_microscope(self, routine: Literal['image', 'focus', 'expose'],
                           roi: Union[ROI, List[ROI]] = [], 
    ...
    def image(self, roi: Union[ROI, List[ROI]] = [], 
                    flowcells: Union[str, List[str]] = None, **kwargs):
        """Image ROIs."""
    ...
    def focus(self, roi: Union[ROI, List[ROI]], 
                    flowcells: Union[str, List[str]] = None):
        """Find focus z position in ROIs."""
        self._roi_to_microscope('focus', roi, flowcells)

    def expose(self, roi: Union[ROI, List[ROI]], 
                     flowcells: Union[str, List[str]] = None, **kwargs):
    ...
    def add_rois(self, flowcells, roi_path:str) -> int:
        ...
        try: 
            rois = read_roi_config(flowcells, roi_path, 
    ...
    async def new_experiment(self, flowcells:str, exp_config_path:str):
        ...
        for fc in flowcells:
            if not self.flowcells[fc]._queue.empty():
                raise RuntimeError(f'Flow cell {fc.name} still running, stop flow cell before starting new experiment')
            else:
        ...
        for fc in flowcells:
            ...
            if missing_reagents > 0:
                raise ValueError(f"Missing {missing_reagents} reagents for flowcell {fc}")
    ...
    def queue_protocol(self, flowcell: Union[str,int], fprotocols:dict):
        ...
        for pname, protocol in fprotocols.items():
            ...
            for cycle in range(protocol['cycles']):
                self.flowcells[flowcell].update_protocol_cycle(cycle+1, protocol['cycles'])
