...
async def BaseTestSequencerWithReagents(BaseTestSequencer):
    ...
    for fc in ['A','B']:
        BaseTestSequencer._reagents_manager.add(flowcell=fc, name='PBS', port=1, flow_rate=500)
        BaseTestSequencer._reagents_manager.add(flowcell=fc, name='water', port=2, flow_rate=500)
        BaseTestSequencer._reagents_manager.add(flowcell=fc, name='temp', port=3, flow_rate=500)
    BaseTestSequencer._reagents_manager.add(flowcell=fc, name='test', port=4, flow_rate=100)
...
def test_add_negcntrl(BaseTestSequencerWithReagents):
    ...
    BaseTestSequencerWithReagents._reagents_manager.add(flowcell='A', name='temp', port=5, flow_rate=100)
    # Try to add reagent with dupliciate port
    BaseTestSequencerWithReagents._reagents_manager.add(flowcell='B', name='test1', port=4, flow_rate=100)
...
def test_update_reagent_flowrate(BaseTestSequencerWithReagents):
    # Update flow_rate
    BaseTestSequencerWithReagents._reagents_manager.update(flowcell='B', name='test', flow_rate=200)
    assert BaseTestSequencerWithReagents.flowcells['B'].reagents['test']['flow_rate'] == 200

def test_update_reagent_name(BaseTestSequencerWithReagents):
    # Update name
    BaseTestSequencerWithReagents._reagents_manager.update(flowcell='B', name='test1', port=4)
...
def test_update_reagent_port(BaseTestSequencerWithReagents):
    # Update port
    BaseTestSequencerWithReagents._reagents_manager.update(flowcell='B', name='test', port=5)
...
def test_update_negcntrl(BaseTestSequencerWithReagents):
    ...
    BaseTestSequencerWithReagents._reagents_manager.update(flowcell='B', name='test', port=1)
    ...
    BaseTestSequencerWithReagents._reagents_manager.update(flowcell='B', name='temp', port=4)
    assert BaseTestSequencerWithReagents.flowcells['B'].reagents['test']['port'] == 4
...
def test_remove_reagent(BaseTestSequencerWithReagents):
    BaseTestSequencerWithReagents._reagents_manager.remove(flowcell='A', reagent_name='temp')
    BaseTestSequencerWithReagents._reagents_manager.remove(flowcell='B', reagent_name='temp')
    BaseTestSequencerWithReagents._reagents_manager.remove(flowcell='B', reagent_name='test')
...
def test_remove_negntrl(BaseTestSequencerWithReagents):
    BaseTestSequencerWithReagents._reagents_manager.remove(flowcell='A', reagent_name='test')
    assert len(BaseTestSequencerWithReagents.flowcells['A'].reagents) == 3
